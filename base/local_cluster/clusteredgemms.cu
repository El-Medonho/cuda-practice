...
const int loops = 10;
const float eps = 0.1f*loops;

__global__ void GMM_hard(float *A, float *B, float *C, int n, int k, int m){
...
signed main(int argc, char** argv){
    long long n = 4096, k = 32768, m = 4096;
    ...
    if(rank == 0) {
        vector<float> Ct(n*m, 0);
        long long N = n-softSize;

        vector<float> Cl(softSize*m, 0);
        MPI_Request request; 

        MPI_Irecv(Cl.data(), softSize * m, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, &request);
        ...
        cudaMemset(Cd, 0, N*m*sizeof(float));
        
        double start_gpu = MPI_Wtime();
        for(int g = 0; g < loops; g++){
            for(int i = softSize, ii = 0; i < n; i++, ii++){
                for(int j = 0; j < k; j++) {
                    A[ii*k+j] = ((seed*i*k*g+j)%seed3-half)/(seed3*10);
                }
            }
            for(int i = 0; i < k; i++){
                for(int j = 0; j < m; j++) {
                    B[i*m+j] = ((seed2*i*k*g+j)%seed3-half)/(seed3*10);
                }
            }
            ...
            cout << "[Rank " << rank << "] Finished loop " << g << ' ' << loops << ' ' << (loops >= g) << endl;
        }
        ...
        cout << "[Rank 0 - RTX 5060 Ti] GPU calculating time: " << (end_gpu - start_gpu) << " seconds\n";
        ...
        dim3 grid_base(n/128, m/64);
        cudaMalloc(((void**) &Ad_base), n*k*sizeof(float));
        cudaMalloc(((void**) &Bd_base), m*k*sizeof(float));
        cudaMalloc(((void**) &Cd_base), n*m*sizeof(float));
        cudaMemset(Cd_base, 0, n*m*sizeof(float));

        double start_base = MPI_Wtime();
        for(int g = 0; g < loops; g++){
            for(int i = 0; i < n; i++){
                for(int j = 0; j < k; j++) {
                    A_base[i*k+j] = ((seed*i*k*g+j)%seed3-half)/(seed3*10);
                }
            }
            for(int i = 0; i < k; i++){
                for(int j = 0; j < m; j++) {
                    B_base[i*m+j] = ((seed2*i*k*g+j)%seed3-half)/(seed3*10);
                }
            }
            ...
            GMM_hard<<<grid_base, block>>>(Ad_base, Bd_base, Cd_base, n, k, m);
            cudaDeviceSynchronize();
        }
        ...
        float maxDiff = 0;
        for(int i = 0; i < n*m; i++){
            maxDiff = max(maxDiff, abs(Ct[i] - C_base[i]));
        }
        ...
        vector<float> Al(softSize*k), Bl(k*m), Cl(softSize*m, 0);
        float *Ad, *Bd, *Cd;
        
        cudaMalloc(((void**) &Ad), softSize*k*sizeof(float));
        cudaMalloc(((void**) &Bd), m*k*sizeof(float));
        cudaMalloc(((void**) &Cd), softSize*m*sizeof(float));
        cudaMemset(Cd, 0, softSize*m*sizeof(float));
        
        double start_gpu = MPI_Wtime();
        for(int g = 0; g < loops; g++){
            for(int i = 0; i < softSize; i++){
                for(int j = 0; j < k; j++) {
                    Al[i*k+j] = ((seed*i*k*g+j)%seed3-half)/(seed3*10);
                }
            }
            for(int i = 0; i < k; i++){
                for(int j = 0; j < m; j++) {
                    Bl[i*m+j] = ((seed2*i*k*g+j)%seed3-half)/(seed3*10);
                }
            }
            
            cudaMemcpy(Ad, Al.data(), softSize*k*sizeof(float), cudaMemcpyHostToDevice);
            cudaMemcpy(Bd, Bl.data(), m*k*sizeof(float), cudaMemcpyHostToDevice);
            dim3 softGrid(softSize/64, m/64), softBlock(64, 8);
    
            GMM_soft<<<softGrid, softBlock>>>(Ad, Bd, Cd, softSize, k, m);
            cudaDeviceSynchronize();
            MPI_Barrier(MPI_COMM_WORLD);
            
            cout << "[Rank " << rank << "] Finished loop " << g << ' ' << loops << ' ' << (g >= loops) << endl;
        }
        double end_gpu = MPI_Wtime();
        cout << "oi2" << endl;


        cudaMemcpy(Cl.data(), Cd, softSize*m*sizeof(float), cudaMemcpyDeviceToHost);

        MPI_Send(Cl.data(), softSize * m, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
